...
2. 【最重要】使用体験を書かない。
   この記事の書き手は、その商品を使っていない。したがって
   「実際に使ってみると」「筆者は」「試したところ」「使ってみて感じたのは」
   のような一人称の体験表現を一切使ってはいけない。
   期間・回数・体感の変化など、経験したことにしか書けない具体を書かない。
   読者はこれを読んで金を使うため、経験していないことを経験したように書くのは嘘になる。

3. 価格を断定しない。「〇〇円です」ではなく「〇〇円台から選べます」のように幅で書く。
4. 誇大な断定を避ける。「絶対に」「必ず」「No.1」は使わない。
5. 健康・医療・金銭的効果に関する主張はしない。
6. 読者が自分で判断できる基準を書く。何を見て選べばよいかを具体的に示す。
7. 検索順位のための水増しをしない。内容のない一般論で文字数を稼がない。

あなたが書けるのは「一般に知られている選び方の知識」だけです。
それを、読者が商品一覧を見たときに自分で判断できる形に整理してください。
"""
...
## この記事に自動で差し込まれるもの
- `<!--STATS-->` … 楽天市場の該当商品を数十件集計した統計（価格の中央値、価格帯ごとの平均評価、送料無料率など）が読者のブラウザで計算されて表示されます。
- `<!--TABLE-->` … 実際の商品一覧が、レビュー数順・価格順・評価順で切り替えられる形で表示されます。

**あなたには具体的な商品も数値も分かりません。** だから商品名も価格も書いてはいけません。
あなたの仕事は、読者がその表を見たときに「自分はどれを選ぶべきか」を判断できる
**基準と考え方**を先に与えることです。

## 必須の構成

```
（リード文：3〜4文。読者が今どんな状況で困っているかを具体的に描写する。
　「〜という経験はありませんか」のような問いかけは使わない。見出しなし）

## {keyword}の選び方

（判断基準を3つ。それぞれ `### ` を付けて4〜6文。
　各基準について次の3点を必ず含める:
   1. 何を見るのか（商品ページのどの項目か）
   2. どの数値・仕様が境目になるのか（可能な限り具体的な数字を出す）
   3. その境目を外すとどうなるのか）

## 用途別の選び方

（読者のタイプを3つに分け、それぞれ `### ` を付けて2〜3文。
　「予算を抑えたい人」「長時間使う人」「省スペース重視の人」のように、
　先に挙げた基準のどれを優先すべきかを示す）

## 失敗しやすいポイント

（3つ。`### ` を付けて3〜4文ずつ。
　「買ってから気づく」類の見落としを、なぜ見落とすのかとセットで説明する）

## 購入前に確認したいこと

（箇条書き5〜7項目。商品ページで実際に確認すべき項目名を挙げる。
　保証期間、対応環境、サイズ、付属品など、キーワードに応じて適切なものを選ぶ）

## 楽天市場のデータから分かること

（2〜3文の導入。「以下は楽天市場の該当商品を集計したものです」のように、
　次に出てくる数値が何を示すかを説明する。具体的な数値は書かない）

<!--STATS-->

## 現在の人気商品

（1〜2文。並び順を切り替えられることに触れる）

<!--TABLE-->

## まとめ

（4〜5文。どういう条件の人がどの基準を優先すべきかを整理する。
　特定の商品名は挙げない）

## よくある質問

（Q&A形式で5組。`### Q. ` と回答。
　読者が購入前に実際に迷うこと、専門用語の意味、互換性や設置の可否などを扱う）
```

## 厳守事項
- **具体的な商品名・型番・メーカー名を一切書かない**
- **使用体験を書かない。「実際に使ってみると」「筆者は」等の表現を使わない**
- **`<!--STATS-->` と `<!--TABLE-->` は必ずそのまま1行で残す**
- 価格は幅で書く（「5000円前後から」など）。断定しない
- 一般論で字数を稼がない。数値・仕様・判断基準など、読者が確認に使える具体を書く
- 記事タイトルは出力しない（本文のみ）
- 全体で3500〜4500字
...
# 一人称の使用体験を示す表現。生成結果にこれが出たら警告する。
EXPERIENCE_WORDS = [
    "実際に使って", "使ってみると", "使ってみて", "試してみ", "筆者",
    "私が使", "愛用し", "購入して", "使い始めて", "使い続け",
]
...
以下は楽天市場の該当商品を数十件集計したものです。\
価格の分布や、価格帯ごとの評価の違いを確認できます。\
数値はこのページを開いた時点のものです。
...
以下は楽天市場の検索結果です。並び順はレビュー数・価格・評価で切り替えられます。\
...
- 重量が商品ページに明記されているか（記載のない製品は実測値がばらつくことがある）
- 接続方式（有線 / 2.4GHz無線 / Bluetooth）と、無線なら電池式か充電式か
- ボタン数と、割り当て変更に専用ソフトが必要かどうか
- 保証期間と、国内正規品かどうか
...
def main(argv: list[str] | None = None) -> None:
    ...
    # 生成が指示を外した場合の保険。ブロックが消えるより末尾に付くほうがまし。
    for marker in ("<!--STATS-->", "<!--TABLE-->"):
        if marker not in body:
            print(f"  [警告] {marker} が本文にありません。末尾に追加します。")
            body += f"\n\n{marker}\n"

    # --- 比較表と集計の置き場所（中身はブラウザが埋める） ---
    search_kw = entry.get("search_keyword") or keyword
    ...
    # 生成物に一人称の体験表現が混じっていないか検査する。
    # プロンプトで禁じていても、まれに書いてしまうことがある。
    # 使っていない商品の体験談は読者を誤認させるため、ここで気づけるようにする。
    suspect = [w for w in EXPERIENCE_WORDS if w in body]
    if suspect:
        print(f"  [警告] 体験を示す表現が含まれています: {suspect}")
        print("         実際に使っていないなら、公開前に該当箇所を直してください。")
    # GitHub Actions の後続ステップに draft_id を渡す
    import os
    auto = bool(config.get("auto_publish", False))
    print(f"  自動公開: {'有効' if auto else '無効（承認メールを送ります）'}")
    if out := os.environ.get("GITHUB_OUTPUT"):
        with open(out, "a", encoding="utf-8") as fh:
            fh.write(f"draft_id={draft_id}\n")
            fh.write(f"auto_publish={'true' if auto else 'false'}\n")
